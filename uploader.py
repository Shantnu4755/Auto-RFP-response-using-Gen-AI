...
UPLOAD_FOLDER = 'backend/source_documents'
ALLOWED_EXTENSIONS = {'txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif'}
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        # Call `ingest.py` and wait for it
        process = subprocess.Popen(['python3', 'backend/ingest.py'], stdout=subprocess.PIPE)
        for line in process.stdout:
            print(line.decode().strip())
            if line.decode().strip() == "Ingestion complete! You can now run privateGPT.py to query your documents":
                break
        ...
        # Call `privateGPT.py` after ingest.py
        process = subprocess.Popen(['python3', 'backend/privateGPT.py'], stdout=subprocess.PIPE)
        output, _ = process.communicate()
        process.wait()

        return jsonify({'message': 'File uploaded and processed', 'output': output.decode().strip()})

    return jsonify({'error': 'File type not allowed'}), 400

@app.route('/download-ppt', methods=['GET'])
def download_ppt():
    ppt_path = 'path/to/generated.pptx'  # Adjust the path to your generated PPT file
